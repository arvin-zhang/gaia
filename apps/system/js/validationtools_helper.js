...
*
* This file is a temporary solution for validationTools app to hack special events of
* hardware-button. The current architecture of gecko only supports reporting those events
* to system module and no loger dispatching to other gaia apps. Therefore, we have to
* establish an customized channel so as to satisfy the requirement of validationTools.
*
*/
...
(function (exports) {
	...
	var ValidationToolsHelper = function(){
		...
		// the identifier is to ensure the fault-tolerant especially on some abnormal
		// conditions such as validationtools app was exceptionally killed with no 
		// chance to terminate the monitoring
	...
	ValidationToolsHelper.prototype = {
		...
		debug: function vth_debug() {
	    	if (DEBUG) {
				console.log('[ValidationToolsHelper]' + 
							'[' + Date.now() + ']' +
		...
		handleEvent: function vth_handleEvent(evt) {
			...
			// if current app is not validationtools and portEnable is true, we
			// judge that validationtools app has been killed abnormally and then
